...
Double_t ErrDiv(Float_t a, Float_t da, Float_t b, Float_t db);
...
void Measurement_3pts(TString const& filename1,TString const& filename2,TString const& filename3,int nevents=0,double resbin=0.3, double xmin=0, double xmax=15){
	
        int n_bins = (xmax - xmin )/resbin;

        //Float_t position[3]  = {0.597, 1.343, 1.994}; // meters
        //Float_t eposition[3] = {0.001, 0.001, 0.001};
        //Float_t position[3]  = {1.400, 1.631, 1.997}; // good (meters)
        Float_t position[3]  = {1.400, 1.631, 1.997}; // meters
        Float_t correction[3]  = {0.00951127,0.0082527, 0.00681845}; // meters
        Float_t eposition[3] = {0.01, 0.01, 0.01};
	...
	for (Long64_t i=0; i < nentries; ++i) {
        ...
        cout << "for FIT : " << mean << " " << sigma << " " << x_min << " " << x_max << endl;
	...
	fit_function1->SetParameter(0,1000);
	fit_function1->SetParameter(1,mean);
	fit_function1->SetParameter(2,sigma);
	fit_function1->SetParameter(3,1000);
	fit_function1->SetParameter(4,mean);
	fit_function1->SetParameter(5,3*sigma);
	fit_function1->SetParameter(6,20000);
	fit_function1->SetParameter(7,mean);
	fit_function1->SetParameter(8,5*sigma);

	fit_function1->SetParLimits(0,0,100000);
	fit_function1->SetParLimits(1,0,14);
	fit_function1->SetParLimits(2,0,2);
	fit_function1->SetParLimits(3,0,100000);
	fit_function1->SetParLimits(4,0,14);
	fit_function1->SetParLimits(5,0,100);
	fit_function1->SetParLimits(6,0,1000000000);
	fit_function1->SetParLimits(7,0,10000);
	fit_function1->SetParLimits(8,0,1000000000);

	//fit_function1->FixParameter(3,0);
	//fit_function1->FixParameter(4,0);
	//fit_function1->FixParameter(5,1.);
	fit_function1->FixParameter(6,0);
	fit_function1->FixParameter(7,0);
	fit_function1->FixParameter(8,1.);

        //hist1->SetMarkerColor(1);
        //hist1->SetMarkerStyle(1);
        //hist1->SetMarkerSize(4);
	...
	hist1->Fit(fit_function1,"R");
        ...
        err_par6 = fit_function1->GetParhttp://www.cplusplus.com/reference/cstdio/sprintf/Error(6);
	...
	for (Long64_t i=0; i < nentries; ++i) {
        ...
        x_min = mean - n_sigmas*sigma; 
        x_max = mean + n_sigmas*sigma;
        if (x_min<xmin) x_min = xmin;http://www.cplusplus.com/reference/cstdio/sprintf/
        if (x_max<xmax) x_max = xmax;
	...
	fit_function2->SetParameter(0,1000);
	fit_function2->SetParameter(1,mean);
	fit_function2->SetParameter(2,sigma);
	fit_function2->SetParameter(3,100);
	fit_function2->SetParameter(4,mean);
	fit_function2->SetParameter(5,3*sigma);
	fit_function2->SetParameter(6,20000);
	fit_function2->SetParameter(7,mean);
	fit_function2->SetParameter(8,5*sigma);
        
	fit_function2->SetParLimits(0,0,100000);
	fit_function2->SetParLimits(1,0,14);
	fit_function2->SetParLimits(2,0,2);
	fit_function2->SetParLimits(3,0,10000);
	fit_function2->SetParLimits(4,0,14);
	fit_function2->SetParLimits(5,0,100);
	fit_function2->SetParLimits(6,0,1000000000);
	fit_function2->SetParLimits(7,0,10000);
	fit_function2->SetParLimits(8,0,1000000000);

	//fit_function2->FixParameter(3,0);
	//fit_function2->FixParameter(4,0);
	//fit_function2->FixParameter(5,1.);
	fit_function2->FixParameter(6,0);
	fit_function2->FixParameter(7,0);
	fit_function2->FixParameter(8,1.);
	...
	hist2->Fit(fit_function2,"R","");
        ...
        par3     = fit_function2->GetParameter(3);
        err_par3 = fit_function2->GetParError(3);
        par4     = fit_function2->GetParameter(4);
        err_par4 = fit_function2->GetParError(4);
        par5     = fit_function2->GetParameter(5);
        err_par5 = fit_function2->GetParError(5);
	...
	fit_function3->SetParameter(0,500);
	fit_function3->SetParameter(1,mean);
	fit_function3->SetParameter(2,sigma);
	fit_function3->SetParameter(3,5000);
	fit_function3->SetParameter(4,mean);
	fit_function3->SetParameter(5,3*sigma);
	fit_function3->SetParameter(6,20000);
	fit_function3->SetParameter(7,mean);
	fit_function3->SetParameter(8,5*sigma);
      
	fit_function3->SetParLimits(0,0,10000);
	fit_function3->SetParLimits(1,0,14);
	fit_function3->SetParLimits(2,0,2);
	fit_function3->SetParLimits(3,0,10000);
	fit_function3->SetParLimits(4,0,14);
	fit_function3->SetParLimits(5,0,100);
	fit_function3->SetParLimits(6,0,1000000000);
	fit_function3->SetParLimits(7,0,10000);
	fit_function3->SetParLimits(8,0,1000000000);

	//fit_function3->FixParameter(3,0);
	//fit_function3->FixParameter(4,0);
	//fit_function3->FixParameter(5,1.);
	fit_function3->FixParameter(6,0);
	fit_function3->FixParameter(7,0);
	fit_function3->FixParameter(8,1.);
	...
	TCanvas *c1 = new TCanvas("c1","",100,9,700,700);
   	...
   	//c1->GetFrame()->SetBorderSize(12);
        ...
        // /*
        Int_t num = 3;
        cout << "debug = " << position[0] << endl;
        /*
        for (int i=0; i++,i<num){
            position_corrected[i] = position[i]+correction[i];
        }
        */
        //TGraphErrors *gr = new TGraphErrors(num, position_corrected, time, eposition, etime);
        TGraphErrors *gr = new TGraphErrors(num, position, time, eposition, etime);
        gr->SetTitle("muon speed");
 	gr->GetYaxis()->SetTitle("#Delta time (ns)");
   	gr->GetXaxis()->SetTitle("#Delta position (m)");
        ...
        cout << " --------------- "<< endl;
        cout << " muon velocity (10^8 m/s) = " << 10.0/par1 << " +- " << ErrDiv(10,0,par1,err_par1) << endl;   

        Float_t meas_c    = 2.99; 
        Float_t best_par1 = 10.0/meas_c;
 
        TF1* fun_best = new TF1("fun_best","pol1",2);
        fun_best->SetParameter(0,par1); 	
        fun_best->SetParameter(1,best_par1); 
        fun_best->SetLineColor(4);
        fun_best->Draw("SAME");

        // */      
}

void Measurement_4pts(TString const& filename1,TString const& filename2,TString const& filename3,TString const& filename4,double resbin=300, double xmin=0, double xmax=15000){
        ...
        Float_t position[4]  = {0.366, 0.597, 1.343, 1.994}; // meters
        Float_t eposition[4] = {0.001, 0.001, 0.001, 0.001};
	...
	file = new TFile(filename4,"read");
   	tree = (TTree*)file->Get("t1");
	tree->SetBranchAddress("event", &event, &b_event);
  	tree->SetBranchAddress("tdc", &tdc, &b_tdc);

	TH1F* hist4 = new TH1F("hist4", "Events X TDC", n_bins, xmin, xmax);
        hist4->Sumw2(true);
	...
	for (Long64_t i=0; i < nentries; ++i) {
        ...
        mean  = hist4->GetMean();
        sigma = hist4->GetStdDev();
        x_min = mean - n_sigmas*sigma; 
        x_max = mean + n_sigmas*sigma;

        cout << " debug = " << x_min << " " << x_max << endl;
 
        TF1* fit_function4 = new TF1("fit_function4",fun_tripleGauss,x_min,x_max,9);
        TF1* fun4_g0 = new TF1("fun4_g0",fun_Gauss,x_min,x_max,3);
        TF1* fun4_g1 = new TF1("fun4_g1",fun_Gauss,x_min,x_max,3);
        TF1* fun4_g2 = new TF1("fun4_g2",fun_Gauss,x_min,x_max,3);

        /*
	fit_function4->SetParameter(0,10000000);
	fit_function4->SetParameter(1,3000);
	fit_function4->SetParameter(2,1000);
	fit_function4->SetParameter(3,1000000);
	fit_function4->SetParameter(4,5000);
	fit_function4->SetParameter(5,500);
	fit_function4->SetParameter(6,10000);
	fit_function4->SetParameter(7,7000);
	fit_function4->SetParameter(8,10000);
        */

	fit_function4->SetParLimits(0,0,1000000000);
	fit_function4->SetParLimits(1,0,10000);
	fit_function4->SetParLimits(2,0,1000000000);
	fit_function4->SetParLimits(3,0,1000000000);
	fit_function4->SetParLimits(4,0,10000);
	fit_function4->SetParLimits(5,0,1000000000);
	fit_function4->SetParLimits(6,0,1000000000);
	fit_function4->SetParLimits(7,0,10000000);
	fit_function4->SetParLimits(8,0,1000000000);

	fit_function4->FixParameter(6,0);

	hist4->Fit(fit_function4,"R","");
        
        chi2     = fit_function4->GetChisquare(); 
        ndf      = fit_function4->GetNDF();

        par0     = fit_function4->GetParameter(0);
        err_par0 = fit_function4->GetParError(0);
        par1     = fit_function4->GetParameter(1);
        err_par1 = fit_function4->GetParError(1);
        par2     = fit_function4->GetParameter(2);
        err_par2 = fit_function4->GetParError(2);

        par3     = fit_function4->GetParameter(3);
        err_par3 = fit_function4->GetParError(3);
        par4     = fit_function4->GetParameter(4);
        err_par4 = fit_function4->GetParError(4);
        par5     = fit_function4->GetParameter(5);
        err_par5 = fit_function4->GetParError(5);

        par6     = fit_function4->GetParameter(6);
        err_par6 = fit_function4->GetParError(6);
        par7     = fit_function4->GetParameter(7);
        err_par7 = fit_function4->GetParError(7);
        par8     = fit_function4->GetParameter(8);
        err_par8 = fit_function4->GetParError(8);

        fun4_g0->SetParameter(0,par0); 	
        fun4_g0->SetParameter(1,par1); 	
        fun4_g0->SetParameter(2,par2); 	
        fun4_g1->SetParameter(0,par3); 	
        fun4_g1->SetParameter(1,par4); 	
        fun4_g1->SetParameter(2,par5); 	
        fun4_g2->SetParameter(0,par6); 	
        fun4_g2->SetParameter(1,par7); 	
        fun4_g2->SetParameter(2,par8); 	
	...
	TCanvas *c0 = new TCanvas("c0","",100,9,1200,1200);
   	//c0->SetFillColor(42);
   	//c0->SetGrid();
   	//c0->GetFrame()->SetFillColor(21);
   	//c0->GetFrame()->SetBorderSize(12);
 	...
 	hist4->GetYaxis()->SetTitle("Events");
   	hist4->GetXaxis()->SetTitle("#Delta t (ps)");
   	hist4->SetTitle("Events X TDC");	
   	hist4->Draw(); 
        fun4_g0->SetLineColor(4);
        fun4_g0->Draw("SAME");
        fun4_g1->SetLineColor(6);
        fun4_g1->Draw("SAME");
        fun4_g2->SetLineColor(3);
        fun4_g2->Draw("SAME");
...
Double_t ErrDiv(Float_t a, Float_t da, Float_t b, Float_t db)
{
  ...
  Double_t val = (1/b)*(1/b)*TMath::Sqrt((b*b*da*da) + (a*a*db*db));
  return val;
