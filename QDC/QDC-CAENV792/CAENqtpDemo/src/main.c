...

{
    ...
    if ( !cvt_V792_set_pedestal(&board_data, pedestal_value) ) {
       ...
       ret_val = -5;
       goto exit_point;
    } else TRACE1( " (Pedestal value = %i)", pedestal_value );
    TRACE(" Ok\n");
    ...
    // Allocate histogram storage
    {
        ...
        for ( i= 0; i< MAX_HISTO_CHANNELS; i++) {
            histo_buff[i] = (long unsigned int*)malloc( MAX_HISTO_SAMPLES* sizeof( *histo_buff[i]));
    ...
    while( ( read_events < user_setting.m_num_events ) || ( user_setting.m_num_events <= 0 ) ) {
	   ...
	   while ( paused ) {
        ...
        // Process the data
        UINT32 STR_MAX_SIZE = 5*1024*1024;
        //char str_out[STR_MAX_SIZE];
        {
            ...
            while ( ( ( data_size >= 4 ) ) &&
                ...
                if( !silent ) TRACE2("Event/Data size: %lu %u\n",read_events,data_size);
		...
		int values[32]; for(int k = 0; k < 32; ++k) values[k] = 0; 
		...
		if( ( strlen(str_out) + strlen(str_line) ) < STR_MAX_SIZE ) strcat(str_out,str_line);
                else { TRACE("Reached string buffer limit.\n"); } 
                */
		...
		switch ( data & CVT_QTP_DATA_TYPE_MSK ) {
		   ...
		   case CVT_QTP_DATUM: 
			 ...
			 if( ch == channel_output ) sprintf( line_analysis, CHANNEL_COUNT_STR, ch, adc );
