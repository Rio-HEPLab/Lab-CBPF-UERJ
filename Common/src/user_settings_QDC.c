...
// File local defines
////////////////////////////////////////////
#define DEF_RAW_OUTPUT_FILENAME					"output/raw_QDC.dat"					/*!< Raw board values default filename */
#define DEF_PARSED_OUTPUT_FILENAME				"output/parsed_values_TDC.txt"				/*!< Parsed board values default filename */
#define DEF_ANALYSIS_OUTPUT_FILENAME				"output/analysis_values_TDC.dat"			/*!< Parsed board values default filename */

#define DEF_NUM_EVENTS						10000							/*!< Default number of events */
#define DEF_BASE_ADDRESS					0x4444							/*!< Default boards' base address */
...
#define DEF_CONFIG_FILENAME					""							/*!< Configuration file default filename */
...
static BOOL parse_config_file( user_setting_data_QDC* p_data);
...
/**************************************************
**************************************************/
BOOL user_settings_QDC_open( user_setting_data_QDC* p_data) {
...
/**************************************************
**************************************************/
BOOL user_settings_QDC_close( user_setting_data_QDC* p_data) {
...
/**************************************************
**************************************************/
BOOL user_settings_QDC_parse_input_param( user_setting_data_QDC* p_data, int argc, char* argv[]) {
    ...
    if ( strlen( p_data->m_config_filename ) && !parse_config_file( p_data ) )
        return FALSE;
...
/**************************************************
**************************************************/
BOOL parse_config_file( user_setting_data_QDC* p_data) {
    // char line[ 200];
    char org_line[ 200];
    char *line;
    char org_str[200];
	...
	// V775 ONLY!!!!
        ...
        if (!stricmp(str, "ANALYSIS_OUTPUT_FILENAME")) {
            char value[ MAX_FILENAME_LENGHT];
			value[0]= '\0';
            sscanf( line, "%s", value);
...
BOOL parse_zero_suppression_thresholds( const char* line, UINT16 *thresholds_buff) {
    ...
    strncpy( buff, trim( trim_text_charset, 400, line), 400);
...
BOOL parse_acquisition_mode_param( const char* line, acquisition_mode_param_QDC *target) {
    const char* tok;
    char buff[ 300];
